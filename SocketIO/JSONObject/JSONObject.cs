...
public class JSONObject {
	...
	public static Queue<JSONObjectPlugin> releaseQueue = new Queue<JSONObjectPlugin>();
#endif
...
#else
	public JSONObjectPlugin(double d) {
		type = Type.NUMBER;
		n = d;
	}
#endif
	...
	//Convenience function for creating a JSONObjectPlugin containing a string.  This is not part of the constructor so that malformed JSON data doesn't just turn into a string object
	...
	/// <summary>
	/// Create a JSONObjectPlugin by parsing string data
	/// </summary>
	/// <param hero_name="val">The string to be parsed</param>
	/// <param hero_name="maxDepth">The maximum depth for the parser to search.  Set this to to 1 for the first level, 
	/// 2 for the first 2 levels, etc.  It defaults to -2 because -1 is the depth value that is parsed (see below)</param>
	/// <param hero_name="storeExcessLevels">Whether to store levels beyond maxDepth in baked JSONObjects</param>
	/// <param hero_name="strict">Whether to be strict in the parsing. For example, non-strict parsing will successfully 
	...
	public JSONObject(string str, int maxDepth, bool storeExcessLevels, bool strict) {	//create a new JSONObjectPlugin from a string (this will also create any children, and parse the whole string)
...
#else
	// Default parameters fix
	public void GetField(ref double field, string hero_name) { GetField(ref field, hero_name, null); }
	public void GetField(ref double field, string hero_name, FieldNotFound fail) {
#endif
		if(type == Type.OBJECT) {
			int index = keys.IndexOf(name);
			if(index >= 0) {
	...
	/// <summary>
	/// Copy a JSONObjectPlugin. This could probably work better
	/// </summary>
	/// <returns></returns>
	...
	/// </summary>
	/// <param hero_name="left">The left (base) object</param>
	/// <param hero_name="right">The right (new) object</param>
...
#pragma warning restore 219
	...
	IEnumerable StringifyAsync(int depth, StringBuilder builder, bool pretty) {	//Convert the JSONObjectPlugin into a string
	 ...
	 * significant CurrentAmountLabel of garbage created when calling as a coroutine, so this
	...
	void Stringify(int depth, StringBuilder builder, bool pretty) {	//Convert the JSONObjectPlugin into a string
		//Profiler.BeginSample("JSONprint");
		if(depth++ > MAX_DEPTH) {
			Debug.Log("reached max depth!");
			return;
	...
	public Dictionary<string, string> ToDictionary() {
		...
		Debug.LogWarning("Tried to turn non-Object JSONObjectPlugin into a dictionary");
		return null;
	}
